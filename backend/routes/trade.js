...
// //sanket - Supports both MongoDB ObjectId AND MetaAPI string accountId (e.g. '91120439')
router.get('/open/:tradingAccountId', async (req, res) => {
  try {
    let { tradingAccountId } = req.params

    // If not a valid ObjectId, look up the TradingAccount by its string accountId
    const isObjectId = /^[a-f\d]{24}$/i.test(tradingAccountId)
    if (!isObjectId) {
      const account = await TradingAccount.findOne({ accountId: tradingAccountId })
      if (!account) {
        return res.json({ success: true, trades: [] }) // No account found, return empty
      }
...
// //sanket - Supports both MongoDB ObjectId AND MetaAPI string accountId
router.get('/pending/:tradingAccountId', async (req, res) => {
  try {
    ...
    if (!isObjectId) {
      const account = await TradingAccount.findOne({ accountId: tradingAccountId })
      if (!account) return res.json({ success: true, trades: [] })
      tradingAccountId = account._id
    }
