...
class TradeEngine {
  ...
  getContractSize(symbol) {
    // Handle both suffixed and non-suffixed symbols
    const cleanSymbol = symbol.endsWith('.i') ? symbol.slice(0, -2) : symbol
    
    // Metals - 100 oz for gold, 5000 oz for silver
    if (cleanSymbol === 'XAUUSD') return 100
    if (cleanSymbol === 'XAGUSD') return 5000
    // Crypto - 1 unit
    if (['BTCUSD', 'ETHUSD', 'LTCUSD', 'XRPUSD', 'BCHUSD', 'BNBUSD', 'SOLUSD', 'DOGEUSD'].includes(cleanSymbol)) return 1
  ...
  isMarketOpen(symbol) {
    ...
    // Handle suffixed symbols
    const cleanSymbol = symbol.endsWith('.i') ? symbol.slice(0, -2) : symbol

    // Crypto markets are always open
    if (['BTCUSD', 'ETHUSD', 'LTCUSD', 'XRPUSD', 'BCHUSD', 'BNBUSD', 'SOLUSD', 'ADAUSD', 'DOGEUSD', 'DOTUSD', 'MATICUSD', 'AVAXUSD', 'LINKUSD'].includes(cleanSymbol)) {
      return true
    }
