...
const TradingPage = () => {
  ...
  const [starredSymbols, setStarredSymbols] = useState(['XAUUSD.i', 'EURUSD', 'GBPUSD'])
    ...
    const unsubscribe = priceStreamService.subscribe('tradingPage', (prices, updated, timestamp) => {
      ...
      if (now - lastUpdateRef.current > 300) { // //sanket - Throttle to 300ms for silky smooth UI
        ...
        if (selectedPrice && selectedPrice.bid && selectedPrice.bid > 0) {
          ...
          // //sanket - Dispatch price update event for the chart datafeed
          getMetaApiPriceEvents().dispatchEvent(new CustomEvent("priceUpdate", {
            detail: { 
              symbol: selectedInstrument.symbol, 
              bid: selectedPrice.bid, 
              ask: selectedPrice.ask || selectedPrice.bid, 
              time: selectedPrice.time || new Date().toISOString() 
            }
          }));
        }
  ...
  const getSymbolCategory = (symbol) => {
    if (['XAUUSD', 'XAGUSD', 'XPTUSD', 'XPDUSD', 'XAUUSD.i', 'XAGUSD.i', 'XPTUSD.i', 'XPDUSD.i'].includes(symbol)) return 'Metals'
    if (['BTCUSD', 'ETHUSD', 'LTCUSD', 'XRPUSD', 'BCHUSD', 'BTCUSD.i', 'ETHUSD.i', 'LTCUSD.i', 'XRPUSD.i'].includes(symbol)) return 'Crypto'
    return 'Forex'
  }
  ...
  const getDisplayPrice = (symbol, side, rawBid, rawAsk) => {
    ...
    if (spreadType === 'PERCENTAGE') {
      ...
      } else if (['XAUUSD', 'XAGUSD', 'XAUUSD.i', 'XAGUSD.i'].includes(symbol)) {
        spread = spreadValue / 100 // Metals: cents to dollars
      } else if (['BTCUSD', 'ETHUSD', 'LTCUSD', 'XRPUSD', 'BNBUSD', 'SOLUSD', 'ADAUSD', 'DOGEUSD', 'DOTUSD', 'MATICUSD', 'AVAXUSD', 'LINKUSD', 'BTCUSD.i', 'ETHUSD.i', 'LTCUSD.i', 'BNBUSD.i', 'SOLUSD.i'].includes(symbol)) {
