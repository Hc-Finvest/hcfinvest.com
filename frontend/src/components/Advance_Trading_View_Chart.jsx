...
//           }}
//         >
//           <div style={{ fontSize: "12px" }}>SELL</div>
//           <div style={{ fontSize: "16px" }}>
//             {bid ? bid.toFixed(2) : "0.00"}
//           </div>
//         </button>
...
//             <button
//               onMouseDown={(e) => e.stopPropagation()}
//               onClick={() => setLot((prev) => Math.max(0.01, prev - 0.01))}
//               style={{
//                 background: "#333",
//                 color: "#fff",
//                 border: "none",
...
//             <span style={{ fontWeight: "bold" }}>{lot.toFixed(2)}</span>

//             <button
//               onMouseDown={(e) => e.stopPropagation()}
//               onClick={() => setLot((prev) => prev + 0.01)}
//               style={{
//                 background: "#333",
//                 color: "#fff",
//                 border: "none",
...
const normalizeAuthToken = (raw) => {
  ...
  // Handle accidentally JSON-stringified token values like "\"eyJ...\""
...
const getAuthToken = () => {
  ...
  const sessionDirect = normalizeAuthToken(sessionStorage.getItem("token"));
  ...
  try {
    ...
    if (rawUser) {
      ...
      const fromUser = normalizeAuthToken(user?.token || user?.accessToken || user?.jwt);
      if (fromUser) return fromUser;
    }
  } catch (e) {
    // Ignore parse errors and continue with empty token
  }
...
const Advance_Trading_View_Chart = ({ symbol = "XAUUSD", trades = [], onTradeModify }) => {
  ...
  const tradesRef = useRef(trades); // Keep latest trades for event callbacks (avoid stale closure)
  ...
  const createOrderLineAtPrice = async (price, label, color, lineStyle, tradeId, levelType) => {
    ...
    const isEntryLine = levelType === 'entry';
    ...
    try {
      ...
      if (chartRef.current.createOrderLine && typeof chartRef.current.createOrderLine === 'function') {
        try {
          ...
          orderLine.setLineWidth(isEntryLine ? 3 : 2);
          orderLine.setLineStyle(lineStyle);

          // Keep entry line fixed at execution price; only SL/TP should be draggable.
          if (isEntryLine && typeof orderLine.setEditable === 'function') {
        ...
        try {
          ...
          const shape = await chartRef.current.createShape(
            { time: now, price: price },
            {
              ...
              text: label,
              // Entry is fixed; SL/TP remain user-draggable.
              lock: isEntryLine,
              disableSelection: isEntryLine,
              disableSave: true,
              disableUndo: true,
              overrides: {
                linecolor: color,
                linewidth: isEntryLine ? 3 : 2,
                linestyle: lineStyle,
                showLabel: true,
                textcolor: color,
                fontsize: 12,
  ...
  const processPriceModification = async (shapeId, newPrice) => {
    ...
    const mappedTradeId = String(tradeId);
    const trade = currentTrades.find(t => String(t?._id ?? t?.id ?? '') === mappedTradeId);
    
    if (!trade) {
      dragLogger.error(`Trade not found: ${tradeId} (currentTrades=${currentTrades.length})`);
      return;
    }
    ...
    if (activeDragsRef.current[tradeId]) {
      dragLogger.log('SKIP', `Duplicate drag event ignored for trade ${tradeId}`);
      return;
    }
    ...
    try {
      ...
      if (!token) {
        dragLogger.error('No valid auth token found. Please login again.');
        return;
      }
      ...
      if (response.status === 401) {
        dragLogger.error('Session expired or invalid token. Please re-login and try SL/TP drag again.');
        return;
      }
        ...
        widget.subscribe('drawing_event', async (id, status, object) => {
          ...
          if ((status === 'stopped' || status === 'points_changed' || status === 'move') && id) {
            ...
            try {
              ...
              // Entry line must remain fixed at trade open price.
              if (mapping.levelType === 'entry') {
                dragLogger.log('SKIP', `Ignored drag event on fixed entry line (${eventId})`);
                setTimeout(() => updateTradeLines(), 0);
                return;
              }
      ...
      pollIntervalRef.current = setInterval(() => {
        ...
        try {
          ...
          for (const [lineId, mapping] of Object.entries(lineToTradeMapRef.current)) {
            ...
            if (mapping.levelType === 'entry') continue;
